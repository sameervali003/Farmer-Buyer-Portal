...
const Share = () => {
  ...
  useEffect(() => {
    if (loginStatus && account) {
            axios.get("/api/tool", {
                ...
                console.log(response.data.savedTools)
                setProds(response.data.savedTools)
            })
    }
  }, [loginStatus, account, type])
  ...
  return (
    <>
      <div className="grid md:grid-cols-7 sm:grid-cols-3 xs:grid-cols-2 gap-2 mx-2 mb-10 mt-5">
        {
            tools.map((tool, index) => (
                <button key={index} onClick={() => tool === "All" ? setType() : setType(tool)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    {tool}
                </button>
            ))
        }
      ...
      <div className="grid md:grid-cols-4 sm:grid-cols-3 gap-2 mt-2 mx-2 mb-2 ">
        <div class="px-2 pb-2">
           { 
           prods.map((prod, index) => (
            <ProductCard className="px-5 w-60 h-100" key={index} prod={prod} />
           ))
           }
        </div>
      </div>
